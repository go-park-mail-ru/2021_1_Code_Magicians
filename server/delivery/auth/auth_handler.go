...
type AuthInfo struct {
	...
	boardApp     usecase.BoardAppInterface     // For initial user's board
	websocketApp usecase.WebsocketAppInterface // For setting CSRF token during  login
	logger       *zap.Logger
}

func NewAuthInfo(userApp usecase.UserAppInterface, authApp usecase.AuthAppInterface,
	s3App usecase.S3AppInterface, boardApp usecase.BoardAppInterface,
	websocketApp usecase.WebsocketAppInterface, logger *zap.Logger) *AuthInfo {
	return &AuthInfo{
		userApp:      userApp,
		authApp:      authApp,
		s3App:        s3App,
